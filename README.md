...
- стакан ресурсов на сайте копит снимки в опции WordPress `galia_desk_tape`. Этот чат при новом заходе память обнуляет, поэтому живой график ресурсов держится на walkingyog, не в превью
- ролик под конкретный NFT: в `wordpress/galia-desk/shows.json` строка `"MINT": "https://…/i-dance.mp4"`. Если этот mint есть на ключе, плеер появляется в сейфе, и страница шлёт событие `galia-holdings` для mini-sim
- база 68 карточек экипажа лежит в плагине (`roster.json`) и раскрывается на странице блоком «База экипажа»
