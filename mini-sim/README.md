...
## STOP â read this before another GPT rewrite
...
### Why 0.4.1 looked ânot appliedâ
- PHP was updated to 0.4.1 (`?ver=0.4.1`).
- **JS on the server stayed the old 48890-byte 0.4.0 file.**
- Always check file size / first comment / HUD title `SPACE LABYRINTH â 0.5.2`.
...
1. Music starts **only** on button Â«ÐÐ³ÑÐ°ÑÑÂ». Never on page load / pointerdown / engine init.
...
5. Portal: play the **mp4 overlay**, then teleport ship to room 2 (`z â -52`). If the portal quad covers ~69% of view, auto-start the cutscene.
...
Only these three files (do not re-upload 3â6MB png/mp4 unless missing):
...
- HUD: `ENGINE READY Â· LOCAL r128 Â· 0.5.2`
- Click ÐÐÐ ÐÐ¢Ð¬ â OST starts. Refresh page â OST silent until click.
